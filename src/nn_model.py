...
def proposed():
    ...
    # Layer : Convolutional. Input:32x32x3, Output:30x30x6. (filters=6, kernel_size=(3, 3))
    model.add(tf.keras.layers.Conv2D(6, (3, 3), padding='VALID', activation='relu', input_shape=(32, 32, 3)))

    # Layer : Dropout.
    model.add(tf.keras.layers.Dropout(0.2))

    # Layer : Pooling. Input:30x30x6, Output:28x28x6. (pool_size=(3, 3), strides=(1, 1))
    model.add(tf.keras.layers.MaxPooling2D((3, 3), (1, 1), padding='VALID'))

    # Layer : Convolutional. Input:28x28x6, Output:24x24x11. (filters=14, kernel_size=(5, 5))
    model.add(tf.keras.layers.Conv2D(14, (5, 5), padding='VALID', activation='relu'))

    # Layer : Dropout.
    model.add(tf.keras.layers.Dropout(0.3))

    # Layer : Pooling. Input:24x24x11, Output:12x12x11. (pool_size=(2, 2), strides=(2, 2))
    model.add(tf.keras.layers.MaxPooling2D((2, 2), (2, 2), padding='VALID'))

    # Layer : Convolutional. Input:12x12x11. Output:6x6x18. (filters=22, kernel_size=(7, 7))
    model.add(tf.keras.layers.Conv2D(22, (7, 7), padding='VALID', activation='relu'))

    # Layer : Dropout
    model.add(tf.keras.layers.Dropout(0.5))

    # Layer : Pooling. Input:6x6x18, Output:5x5x22. (pool_size(2, 2), strides=(1, 1))
    model.add(tf.keras.layers.MaxPooling2D((2, 2), (1, 1), padding='VALID'))

    # Layer : Fully Connected. Input:5x5x22, Output:550.
    model.add(tf.keras.layers.Flatten())

    # Layer : Fully Connected. Input:550, Output:150.
    model.add(tf.keras.layers.Dense(150, kernel_regularizer=tf.keras.regularizers.l2(0.003), activation='relu'))

    # Layer : Fully Connected. Input:150, Output:100.
    model.add(tf.keras.layers.Dense(100, kernel_regularizer=tf.keras.regularizers.l2(0.001), activation='relu'))

    # Layer : Fully Connected. Input:100, Output:43.
    model.add(tf.keras.layers.Dense(43, activation='softmax'))
